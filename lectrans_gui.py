...
class LecTransApp:
    ...
    def create_header(self, parent):
        ...
        ttk.Label(header, text="实时课堂翻译工具", font=('Segoe UI', 10)).pack(side=LEFT, padx=(10, 0))
        ...
        try:
            ...
            if self.config.asr_api_key:
                try:
                    ...
                    self.recognizer_client = Groq(api_key=self.config.asr_api_key)
                except Exception:
                    pass
    ...
    def create_api_settings(self, parent):
        ...
        ttk.Combobox(asr_frame, textvariable=self.asr_provider_var, values=["groq", "openai"], state="readonly").grid(row=0, column=1, padx=5, pady=5, sticky=EW)
        ...
        ttk.Entry(asr_frame, textvariable=self.asr_key_var, show="*").grid(row=1, column=1, padx=5, pady=5, sticky=EW)
...
© 2024 LecTrans Team
        """
        ttk.Label(parent, text=text, justify=LEFT, font=('Segoe UI', 10)).pack(padx=20, pady=20)
    ...
    def test_connection(self):
        ...
        try:
            from openai import OpenAI
            client = OpenAI(api_key=self.llm_key_var.get(), base_url=self.llm_url_var.get())
