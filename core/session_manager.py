...
支持自动保存、历史浏览、录音文件关联、Markdown 导出。
"""
...
from config import RECORDINGS_DIR, SESSIONS_DIR
...
class SessionManager:
    ...
    def list_sessions(self) -> List[dict]:
        ...
        for fp in SESSIONS_DIR.glob("*.json"):
            try:
                ...
                logger.warning("跳过损坏的会话文件 %s: %s", fp.name, e)
    ...
    def delete_session(self, session_id: str):
        ...
        try:
            data = self.load_session(session_id) or {}
        ...
        for t in data.get("transcripts", []):
            ...
            if ts:
                try:
                    ...
                    pass
            lines.append(f"### [{ts}]")
            lines.append(f"**韩语**: {t.get('korean', '')}\n")
            lines.append(f"**中文**: {t.get('chinese', '')}\n")
            lines.append("---\n")
